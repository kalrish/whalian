...
if [[ ! -v DEB_SIGN_KEYID ]]
then
	echo 'whalian: error: environment variable `DEB_SIGN_KEYID` not defined' >&2
	exit_code=1
fi

if [[ ! -v DEBSIGN_KEYID ]]
then
	echo 'whalian: error: environment variable `DEBSIGN_KEYID` not defined' >&2
	exit_code=1
fi

if [[ ${exit_code} -eq 0 ]]
then
	if [[ ${DEB_SIGN_KEYID} = ${DEBSIGN_KEYID} ]]
	then
		if [[ $@ = ?(-)?(-)help ]]
		then
			show_usage
		...
		echo 'whalian: error: values of environment variables `DEB_SIGN_KEYID` and `DEBSIGN_KEYID` differ' >&2
		exit_code=1
	fi
fi
